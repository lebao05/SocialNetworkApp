/* Admin Panel — core JS
 *
 * Cross-page behaviours that need to be available on every admin view:
 ...
 *
 * Per-page scripts (admin.dashboard.js, admin.users.js, ...) extend
 * window.Admin.<PageName>; this file only owns the global stuff.
 */
(function () {
  'use strict';
    ...
    document.querySelectorAll('.tab').forEach(function (tab) {
      tab.addEventListener('click', function () {
        ...
        var panel = document.querySelector(
          '.tab-panel[data-tab-group="' + group + '"][data-tab="' + target + '"]');
  ...
  /* ── Table Row Selection / Bulk Actions ── */
  Admin.initTables = function () {
    document.querySelectorAll('.table-wrapper').forEach(function (wrapper) {
      var checkboxes = wrapper.querySelectorAll('tbody input[type="checkbox"]');
      var selectAll = wrapper.querySelector('thead input[type="checkbox"]');
      if (!selectAll) return;
      ...
      checkboxes.forEach(function (cb) {
        cb.addEventListener('change', function () { updateBulkActions(wrapper); });
      });
    });
  };

  function updateBulkActions(wrapper) {
    var checked = wrapper.querySelectorAll('tbody input[type="checkbox"]:checked').length;
    var bulkBar = document.querySelector('.bulk-action-bar');
    var countEl = document.querySelector('.bulk-count');
