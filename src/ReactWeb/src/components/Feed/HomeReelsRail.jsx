...
export default function HomeReelsRail() {
  ...
  const { reels, isLoading, error } = useHomeReels(6);
        ...
        {isLoading ? (
          <div className="flex gap-3 overflow-x-auto">
            {[...Array(4)].map((_, i) => (
              <div
                key={i}
                className="aspect-[9/16] w-[150px] min-w-[150px] animate-pulse rounded-2xl bg-gray-200"
              />
            ))}
          </div>
        ) : error || reels.length === 0 ? (
          <div className="flex aspect-[9/16] w-[150px] min-w-[150px] flex-col items-center justify-center rounded-2xl border border-dashed border-[#d8dadf] bg-[#f8fafc] px-4 text-center transition-colors">
                ...
                <div className="absolute inset-x-0 bottom-0 p-3 text-white">
                  <div className="mb-2 flex items-center gap-2">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-[13px] font-bold">{reel.authorName}</p>
                      <p className="truncate text-[11px] text-white/70">{reel.viewCount} views</p>
                    </div>
                  </div>
