// ─── Current User ────────────────────────────────────────────────────────────
export const currentUser = {
  id: 1,
  name: "Minh Khôi",
  avatar: "https://i.pravatar.cc/150?img=8",
  coverPhoto: "https://picsum.photos/seed/cover1/850/315",
};

// ─── Stories ─────────────────────────────────────────────────────────────────
// Each user can have multiple story items
export const storyGroups = [
  {
    id: 1,
    user: "Bảo Châu",
    avatar: "https://i.pravatar.cc/150?img=47",
    stories: [
      { id: 101, bg: "https://picsum.photos/seed/st11/200/350", label: "Đi Đà Lạt 🌸", timestamp: "2 giờ trước" },
      { id: 102, bg: "https://picsum.photos/seed/st12/200/350", label: "Cafe Mê Ly", timestamp: "2 giờ trước" },
    ],
  },
  {
    id: 2,
    user: "Tuấn Kiệt",
    avatar: "https://i.pravatar.cc/150?img=11",
    stories: [
      { id: 201, bg: "https://picsum.photos/seed/st21/200/350", label: "Rank mùa mới 🎮", timestamp: "30 phút trước" },
    ],
  },
  {
    id: 3,
    user: "Hà Linh",
    avatar: "https://i.pravatar.cc/150?img=44",
    stories: [
      { id: 301, bg: "https://picsum.photos/seed/st31/200/350", label: "Cà phê sáng ☕", timestamp: "1 giờ trước" },
      { id: 302, bg: "https://picsum.photos/seed/st32/200/350", label: "Thiết kế mới ✨", timestamp: "1 giờ trước" },
      { id: 303, bg: "https://picsum.photos/seed/st33/200/350", label: "Dạo phố chiều 🌆", timestamp: "1 giờ trước" },
    ],
  },
  {
    id: 4,
    user: "Quốc Bảo",
    avatar: "https://i.pravatar.cc/150?img=15",
    stories: [
      { id: 401, bg: "https://picsum.photos/seed/st41/200/350", label: "Gym ngày 30 🏋️", timestamp: "3 giờ trước" },
      { id: 402, bg: "https://picsum.photos/seed/st42/200/350", label: "Meal prep 🥗", timestamp: "3 giờ trước" },
    ],
  },
  {
    id: 5,
    user: "Ngọc Hân",
    avatar: "https://i.pravatar.cc/150?img=49",
    stories: [
      { id: 501, bg: "https://picsum.photos/seed/st51/200/350", label: "Tốt nghiệp rồi 🎓", timestamp: "5 giờ trước" },
    ],
  },
  {
    id: 6,
    user: "Duy Anh",
    avatar: "https://i.pravatar.cc/150?img=12",
    stories: [
      { id: 601, bg: "https://picsum.photos/seed/st61/200/350", label: "Road trip 🚗", timestamp: "6 giờ trước" },
      { id: 602, bg: "https://picsum.photos/seed/st62/200/350", label: "Biển Nha Trang 🌊", timestamp: "6 giờ trước" },
      { id: 603, bg: "https://picsum.photos/seed/st63/200/350", label: "Homestay view đẹp quá", timestamp: "6 giờ trước" },
    ],
  },
];

// Flat list for backward compat
export const stories = storyGroups.flatMap((g) =>
  g.stories.map((s) => ({ ...s, user: g.user, avatar: g.avatar, groupId: g.id }))
);

// ─── Posts ────────────────────────────────────────────────────────────────────
export const posts = [
  {
    id: 1,
    user: "HCMUT Programming Club",
    avatar: "https://i.pravatar.cc/150?img=60",
    time: "30m",
    privacy: "public",
    content:
      "🚀 Season 3 hackathon registration is open! This year’s theme is AI for Good.\n\nTop 3 teams share prizes worth 50 million VND. Register before April 15 and check the link in the comments below 👇",
    image: "https://picsum.photos/seed/post01/680/400",
    likes: 3241,
    likedBy: ["👍", "❤️", "🔥"],
    comments: 418,
    shares: 256,
    commentsData: [
      {
        id: 101,
        user: "Anna Lee",
        avatar: "https://i.pravatar.cc/150?img=65",
        time: "15m",
        text: "This sounds amazing! Can’t wait to join. 😊",
      },
      {
        id: 102,
        user: "David Tran",
        avatar: "https://i.pravatar.cc/150?img=18",
        time: "20m",
        text: "Great theme. I hope there’s a beginner track too.",
      },
      {
        id: 103,
        user: "Mai Nguyen",
        avatar: "https://i.pravatar.cc/150?img=39",
        time: "30m",
        text: "Just registered! Good luck everyone. 🤞",
      },
    ],
  },
  {
    id: 2,
    user: "Minh Khôi",
    avatar: "https://i.pravatar.cc/150?img=8",
    time: "2h",
    privacy: "friends",
    content:
      "Finally deployed the app to production 😭🎉 Spent a week debugging CORS only to realize one header was missing. Learned a lot from this one! #webdev #reactjs #nodejs",
    image: "https://picsum.photos/seed/post02/680/380",
    likes: 142,
    likedBy: ["❤️", "😂", "👍"],
    comments: 37,
    shares: 8,
    commentsData: [
      {
        id: 104,
        user: "Hannah",
        avatar: "https://i.pravatar.cc/150?img=26",
        time: "1h",
        text: "Congrats! Those deployment bugs are always the worst.",
      },
      {
        id: 105,
        user: "Quoc An",
        avatar: "https://i.pravatar.cc/150?img=41",
        time: "1h",
        text: "Nice work! What did you use for deployment?",
      },
    ],
  },
  {
    id: 3,
    user: "Ha Linh",
    avatar: "https://i.pravatar.cc/150?img=44",
    time: "4h",
    privacy: "public",
    content:
      "The weather today is so beautiful 🌞 I left early to avoid traffic and got to enjoy the sunrise. Feels good to start the day like this. ✨",
    image: "https://picsum.photos/seed/post03/680/420",
    likes: 521,
    likedBy: ["❤️", "😍", "👍"],
    comments: 64,
    shares: 15,
    commentsData: [
      {
        id: 106,
        user: "Linh Nguyen",
        avatar: "https://i.pravatar.cc/150?img=36",
        time: "2h",
        text: "I love mornings like that! 🌅",
      },
      {
        id: 107,
        user: "Phong",
        avatar: "https://i.pravatar.cc/150?img=22",
        time: "3h",
        text: "Enjoy it before the rain shows up. 😄",
      },
    ],
  },
  {
    id: 4,
    user: "UIT Student Union",
    avatar: "https://i.pravatar.cc/150?img=61",
    time: "6h",
    privacy: "public",
    content:
      "📢 Semester 2 exam schedule for 2025-2026 is now available on the official site.\n\nCheck your schedule carefully, especially if any exams overlap. Good luck with studying! 💪📚",
    image: null,
    likes: 1893,
    likedBy: ["👍", "😰", "❤️"],
    comments: 712,
    shares: 445,
    commentsData: [
      {
        id: 108,
        user: "Linh Hoa",
        avatar: "https://i.pravatar.cc/150?img=14",
        time: "5h",
        text: "Thanks for the update! I already saved the schedule.",
      },
      {
        id: 109,
        user: "Tuan Anh",
        avatar: "https://i.pravatar.cc/150?img=23",
        time: "5h",
        text: "The overlap on Thursday is going to be tough.",
      },
    ],
  },
  {
    id: 5,
    user: "Tuan Kiet",
    avatar: "https://i.pravatar.cc/150?img=11",
    time: "8h",
    privacy: "friends",
    content: "Reached Challenger rank after 3 months of grinding!! 🏆🔥 Thanks to the team for the support. Next season, let’s do it again 💪",
    image: "https://picsum.photos/seed/post05/680/400",
    likes: 289,
    likedBy: ["🔥", "👍", "❤️"],
    comments: 93,
    shares: 12,
    commentsData: [
      {
        id: 110,
        user: "Nam",
        avatar: "https://i.pravatar.cc/150?img=28",
        time: "7h",
        text: "Well deserved! Keep climbing. 💯",
      },
      {
        id: 111,
        user: "Lien",
        avatar: "https://i.pravatar.cc/150?img=31",
        time: "8h",
        text: "Let’s team up next season!",
      },
    ],
  },
  {
    id: 6,
    user: "Ngoc Han",
    avatar: "https://i.pravatar.cc/150?img=49",
    time: "12h",
    privacy: "public",
    content:
      "After 4 years of university, I finally have my diploma in hand 😭🎓 Thank you to my family, professors, and friends who were there every step of the way. The next chapter begins now! 🚀",
    image: "https://picsum.photos/seed/post06/680/450",
    likes: 4712,
    likedBy: ["❤️", "🎉", "😍"],
    comments: 623,
    shares: 87,
    commentsData: [
      {
        id: 112,
        user: "Quyen",
        avatar: "https://i.pravatar.cc/150?img=24",
        time: "10h",
        text: "Huge congrats! You deserve it. 🎉",
      },
      {
        id: 113,
        user: "Thao",
        avatar: "https://i.pravatar.cc/150?img=29",
        time: "11h",
        text: "So proud of you. What’s next?",
      },
    ],
  },
];

// ─── Contacts (right sidebar) ────────────────────────────────────────────────
export const groupPosts = [
  {
    id: 9001,
    groupId: 1,
    authorName: "Ha Dat Hung",
    authorAvatarUrl: "https://i.pravatar.cc/150?img=12",
    createdAt: new Date(Date.now() - 4 * 60 * 60 * 1000).toISOString(),
    visibility: 0,
    content: "#pkls Vua co mot tay dua khong may nam lai tai giai dua tu than Isle of Man TT.",
    media: [{ id: 9101, mediaType: "Image", mediaUrl: "https://images.unsplash.com/photo-1558981403-c5f9899a28bc?auto=format&fit=crop&w=1000&q=85" }],
    reactionCounts: [{ reactionType: 0, count: 248 }, { reactionType: 4, count: 96 }, { reactionType: 1, count: 34 }],
    commentCount: 58,
    shares: 21,
    commentsData: [
      {
        id: 9201,
        postId: 9001,
        userName: "Kiet Tran",
        userAvatarUrl: "https://i.pravatar.cc/150?img=18",
        content: "Nghe tin buon qua. Duong dua nay dung la qua khac nghiet.",
        createdAt: new Date(Date.now() - 3 * 60 * 60 * 1000).toISOString(),
        reactionCounts: [{ reactionType: 0, count: 8 }],
        replyCount: 1,
      },
      {
        id: 9202,
        postId: 9001,
        userName: "Duy Pham",
        userAvatarUrl: "https://i.pravatar.cc/150?img=31",
        content: "Respect cac tay dua TT. Dam me nhung rui ro qua lon.",
        createdAt: new Date(Date.now() - 2 * 60 * 60 * 1000).toISOString(),
        reactionCounts: [{ reactionType: 1, count: 5 }],
        replyCount: 0,
      },
    ],
  },
  {
    id: 9002,
    groupId: 1,
    authorName: "Phan Khoi Lon tren cung dan S",
    authorAvatarUrl: "https://i.pravatar.cc/150?img=60",
    createdAt: new Date(Date.now() - 9 * 60 * 60 * 1000).toISOString(),
    visibility: 0,
    content: "Anh em cuoi tuan nay co cung Sai Gon - Ho Tram. Tap trung 5:30 tai cay xang quen, chay dung doi hinh va day du bao ho nhe.",
    media: [
      { id: 9102, mediaType: "Image", mediaUrl: "https://images.unsplash.com/photo-1449426468159-d96dbf08f19f?auto=format&fit=crop&w=1000&q=85" },
      { id: 9103, mediaType: "Image", mediaUrl: "https://images.unsplash.com/photo-1517846693594-1567da72af75?auto=format&fit=crop&w=1000&q=85" },
      { id: 9104, mediaType: "Image", mediaUrl: "https://images.unsplash.com/photo-1558981359-219d6364c9c8?auto=format&fit=crop&w=1000&q=85" },
    ],
    reactionCounts: [{ reactionType: 0, count: 421 }, { reactionType: 2, count: 18 }, { reactionType: 1, count: 73 }],
    commentCount: 112,
    shares: 37,
    commentsData: [
      {
        id: 9203,
        postId: 9002,
        userName: "Minh Biker",
        userAvatarUrl: "https://i.pravatar.cc/150?img=22",
        content: "Cho em xin map voi pace du kien admin oi.",
        createdAt: new Date(Date.now() - 7 * 60 * 60 * 1000).toISOString(),
        reactionCounts: [{ reactionType: 0, count: 3 }],
        replyCount: 0,
      },
      {
        id: 9204,
        postId: 9002,
        userName: "An Nguyen",
        userAvatarUrl: "https://i.pravatar.cc/150?img=44",
        content: "Di tour nho gian cach, dung vuot doan nha moi nguoi.",
        createdAt: new Date(Date.now() - 6 * 60 * 60 * 1000).toISOString(),
        reactionCounts: [{ reactionType: 0, count: 14 }],
        replyCount: 2,
      },
    ],
  },
  {
    id: 9003,
    groupId: 1,
    authorName: "Nguyen Quoc Bao",
    authorAvatarUrl: "https://i.pravatar.cc/150?img=15",
    createdAt: new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString(),
    visibility: 0,
    content: "Moi nguoi cho minh hoi lop sport touring nao on cho Ninja 650, uu tien bam mua tot va di pho nhieu?",
    media: [],
    reactionCounts: [{ reactionType: 0, count: 89 }, { reactionType: 5, count: 2 }],
    commentCount: 46,
    shares: 4,
    commentsData: [
      {
        id: 9205,
        postId: 9003,
        userName: "Long Tran",
        userAvatarUrl: "https://i.pravatar.cc/150?img=17",
        content: "Road 6 on do ban, di mua tu tin hon nhieu.",
        createdAt: new Date(Date.now() - 22 * 60 * 60 * 1000).toISOString(),
        reactionCounts: [{ reactionType: 0, count: 6 }],
        replyCount: 0,
      },
    ],
  },
];

export const contacts = [
  { id: 2, name: "Bảo Châu", avatar: "https://i.pravatar.cc/150?img=47", online: true },
  { id: 3, name: "Tuấn Kiệt", avatar: "https://i.pravatar.cc/150?img=11", online: true },
  { id: 4, name: "Hà Linh", avatar: "https://i.pravatar.cc/150?img=44", online: true },
  { id: 5, name: "Quốc Bảo", avatar: "https://i.pravatar.cc/150?img=15", online: false },
  { id: 6, name: "Ngọc Hân", avatar: "https://i.pravatar.cc/150?img=49", online: true },
  { id: 7, name: "Duy Anh", avatar: "https://i.pravatar.cc/150?img=12", online: false },
  { id: 8, name: "Phương Vy", avatar: "https://i.pravatar.cc/150?img=48", online: true },
  { id: 9, name: "Thanh Long", avatar: "https://i.pravatar.cc/150?img=17", online: false },
  { id: 10, name: "Kim Ngân", avatar: "https://i.pravatar.cc/150?img=45", online: true },
  { id: 11, name: "Hoàng Nam", avatar: "https://i.pravatar.cc/150?img=13", online: false },
];

// ─── Conversations (Messenger) ───────────────────────────────────────────────
export const conversations = [
  {
    id: 1,
    name: "Bảo Châu",
    avatar: "https://i.pravatar.cc/150?img=47",
    lastMessage: "Tối nay đi ăn không mày? 🍜",
    time: "2 phút",
    unread: 3,
    online: true,
    isGroup: false,
    messages: [
      { id: 1, senderId: 2, text: "Ê mày, hôm nay có bài tập lớn không?", time: "09:00" },
      { id: 2, senderId: 1, text: "Có nè, môn CNPM deadline thứ 6", time: "09:02" },
      { id: 3, senderId: 2, text: "Chết rồi tao quên mất 😱", time: "09:03" },
      { id: 4, senderId: 1, text: "Haha cẩn thận đó, thầy trừ điểm nặng lắm", time: "09:04" },
      { id: 5, senderId: 2, text: "Oke làm ngay đây", time: "09:05" },
      { id: 6, senderId: 2, text: "Tối nay đi ăn không mày? 🍜", time: "19:30" },
    ],
  },
  {
    id: 2,
    name: "Tuấn Kiệt",
    avatar: "https://i.pravatar.cc/150?img=11",
    lastMessage: "Bạn: Đỉnh vãi, gg mày 🔥",
    time: "15 phút",
    unread: 0,
    online: true,
    isGroup: false,
    messages: [
      { id: 1, senderId: 3, text: "Tao vừa leo lên thách đấu rồi 😤🏆", time: "18:00" },
      { id: 2, senderId: 1, text: "Đỉnh vãi, gg mày 🔥", time: "18:05" },
      { id: 3, senderId: 3, text: "Tháng sau cày cùng nhau không?", time: "18:06" },
      { id: 4, senderId: 1, text: "Oke vào đội tao đi", time: "18:07" },
    ],
  },
  {
    id: 3,
    name: "Nhóm Đồ Án CNPM 🛠️",
    avatar: "https://i.pravatar.cc/150?img=50",
    lastMessage: "Khôi: Push code lên git chưa mọi người?",
    time: "1 giờ",
    unread: 12,
    online: false,
    isGroup: true,
    members: ["Minh Khôi", "Hà Linh", "Quốc Bảo", "Duy Anh"],
    messages: [
      {
        id: 1,
        senderId: 4,
        text: "Mọi người ơi, task của phần login ai làm rồi chưa?",
        time: "14:00",
        senderName: "Hà Linh",
      },
      { id: 2, senderId: 5, text: "Tao làm xong UI rồi, đang chờ API", time: "14:05", senderName: "Quốc Bảo" },
      { id: 3, senderId: 1, text: "API tao đang viết, chiều có", time: "14:10", senderName: "Minh Khôi" },
      {
        id: 4,
        senderId: 7,
        text: "Anh em nhớ viết unit test nhé, thầy check đó",
        time: "14:30",
        senderName: "Duy Anh",
      },
      { id: 5, senderId: 4, text: "😱 Tao chưa làm test", time: "14:31", senderName: "Hà Linh" },
      { id: 6, senderId: 5, text: "😂😂😂", time: "14:31", senderName: "Quốc Bảo" },
      { id: 7, senderId: 1, text: "Push code lên git chưa mọi người?", time: "15:00", senderName: "Minh Khôi" },
    ],
  },
  {
    id: 4,
    name: "Hà Linh",
    avatar: "https://i.pravatar.cc/150?img=44",
    lastMessage: "Ôn thi chung không mày?",
    time: "2 giờ",
    unread: 1,
    online: true,
    isGroup: false,
    messages: [
      { id: 1, senderId: 4, text: "Mày học tới đâu rồi môn Giải Tích?", time: "16:00" },
      { id: 2, senderId: 1, text: "Tới chương 4 rồi, khó vãi", time: "16:05" },
      { id: 3, senderId: 4, text: "Ôn thi chung không mày?", time: "16:30" },
    ],
  },
  {
    id: 5,
    name: "Quốc Bảo",
    avatar: "https://i.pravatar.cc/150?img=15",
    lastMessage: "Bạn: Nhớ đem sạc dự phòng nha",
    time: "3 giờ",
    unread: 0,
    online: false,
    isGroup: false,
    messages: [
      { id: 1, senderId: 5, text: "Mai đi hackathon mấy giờ?", time: "12:00" },
      { id: 2, senderId: 1, text: "7h sáng nha, đừng trễ", time: "12:01" },
      { id: 3, senderId: 5, text: "Oke, tao đặt báo thức 6h rồi", time: "12:02" },
      { id: 4, senderId: 1, text: "Nhớ đem sạc dự phòng nha", time: "12:03" },
    ],
  },
  {
    id: 6,
    name: "Hội Bạn Cấp 3 🎒",
    avatar: "https://i.pravatar.cc/150?img=51",
    lastMessage: "Linh: Họp lớp tháng 4 mọi người ơi!!",
    time: "4 giờ",
    unread: 5,
    online: false,
    isGroup: true,
    members: ["Minh Khôi", "Ngọc Hân", "Duy Anh", "Kim Ngân", "Hoàng Nam"],
    messages: [
      { id: 1, senderId: 6, text: "Lâu rồi không gặp nhau nhỉ 😢", time: "10:00", senderName: "Ngọc Hân" },
      { id: 2, senderId: 7, text: "Đúng rồi, nhớ hồi cấp 3 quá", time: "10:05", senderName: "Duy Anh" },
      {
        id: 3,
        senderId: 10,
        text: "Chụp hình kỷ yếu mà giờ nhìn lại cứ cười 😂",
        time: "10:10",
        senderName: "Kim Ngân",
      },
      { id: 4, senderId: 11, text: "+1 tao cũng nhớ", time: "10:12", senderName: "Hoàng Nam" },
      { id: 5, senderId: 99, text: "Họp lớp tháng 4 mọi người ơi!!", time: "11:00", senderName: "Linh" },
    ],
  },
  {
    id: 7,
    name: "Ngọc Hân",
    avatar: "https://i.pravatar.cc/150?img=49",
    lastMessage: "Bạn: Chúc mừng tốt nghiệp nha!! 🎉",
    time: "5 giờ",
    unread: 0,
    online: true,
    isGroup: false,
    messages: [
      { id: 1, senderId: 6, text: "Tao nhận được bằng rồi 😭😭", time: "08:00" },
      { id: 2, senderId: 1, text: "Chúc mừng tốt nghiệp nha!! 🎉", time: "08:05" },
      { id: 3, senderId: 6, text: "Cảm ơn mày nhiều lắm ❤️", time: "08:06" },
      { id: 4, senderId: 1, text: "Tiếp tục cố gắng nhé!", time: "08:07" },
    ],
  },
  {
    id: 8,
    name: "Phương Vy",
    avatar: "https://i.pravatar.cc/150?img=48",
    lastMessage: "Bạn: Oke để tao xem lại",
    time: "6 giờ",
    unread: 0,
    online: true,
    isGroup: false,
    messages: [
      { id: 1, senderId: 8, text: "Mày review CV của tao với nhé?", time: "07:00" },
      { id: 2, senderId: 1, text: "Oke để tao xem lại", time: "07:30" },
    ],
  },
  {
    id: 9,
    name: "Duy Anh",
    avatar: "https://i.pravatar.cc/150?img=12",
    lastMessage: "Road trip Tết này đi Phú Quốc không anh em?",
    time: "1 ngày",
    unread: 0,
    online: false,
    isGroup: false,
    messages: [
      { id: 1, senderId: 7, text: "Road trip Tết này đi Phú Quốc không anh em?", time: "Hôm qua" },
      { id: 2, senderId: 1, text: "Nghe hay đó, tính chi phí xem sao", time: "Hôm qua" },
    ],
  },
  {
    id: 10,
    name: "Thanh Long",
    avatar: "https://i.pravatar.cc/150?img=17",
    lastMessage: "Bạn: Hehe oke bro",
    time: "2 ngày",
    unread: 0,
    online: false,
    isGroup: false,
    messages: [
      { id: 1, senderId: 9, text: "Mày có tài liệu môn Mạng Máy Tính không?", time: "2 ngày trước" },
      { id: 2, senderId: 1, text: "Hehe oke bro", time: "2 ngày trước" },
    ],
  },
];

// ─── Ads / Sponsored ─────────────────────────────────────────────────────────
export const ads = [
  {
    id: 1,
    brand: "Coursera",
    description: "Học lập trình miễn phí 7 ngày đầu. Hơn 10,000 khoá học chờ bạn!",
    image: "https://picsum.photos/seed/ad01/100/100",
    url: "coursera.org",
  },
  {
    id: 2,
    brand: "TopCV",
    description: "Tạo CV chuyên nghiệp, ứng tuyển ngàn việc làm IT hấp dẫn.",
    image: "https://picsum.photos/seed/ad02/100/100",
    url: "topcv.vn",
  },
];

// ─── Birthdays ────────────────────────────────────────────────────────────────
export const birthdays = [{ id: 5, name: "Quốc Bảo", avatar: "https://i.pravatar.cc/150?img=15" }];

// ─── Groups shortcuts ─────────────────────────────────────────────────────────
export const groupShortcuts = [
  {
    id: 1,
    name: "CLB Lập Trình HCMUT",
    avatar: "https://i.pravatar.cc/150?img=60",
    newPosts: 4,
  },
  {
    id: 2,
    name: "Hội Sinh Viên UIT",
    avatar: "https://i.pravatar.cc/150?img=61",
    newPosts: 12,
  },
  {
    id: 3,
    name: "Mua Bán Đồ Cũ SV",
    avatar: "https://i.pravatar.cc/150?img=62",
    newPosts: 7,
  },
];
